...
const AdminCategories = ({ subSubCategories: initialSubSubCategories, subCategories: initialSubCategories, categories: initialCategories }) => {
  ...
  const deleteSubSubCategory = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/categories/sub-subcategories/${id}`)
  ...
  const deleteSubCategory = async (id) => {
    try {
      ...
      await Promise.all(subSubCategoriesToDelete.map(subSubCategory => axios.delete(`http://localhost:5000/api/categories/sub-subcategories/${subSubCategory._id}`)));
